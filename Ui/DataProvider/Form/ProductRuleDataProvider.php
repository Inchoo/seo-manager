...
class ProductRuleDataProvider extends \Magento\Ui\DataProvider\AbstractDataProvider
{
    ...
    public function getData()
    {
        ...
        if ($data = $this->loadData()) {
            ...
            $this->dataPersistor->clear('product_meta_rule');
        }
    ...
    /**
     * Load the product meta rule data.
     *
     * @return array
     */
    protected function loadData(): array
    {
        if ($data = $this->dataPersistor->get('product_meta_rule')) {
            return $data;
        }

        if ($entityId = $this->request->getParam($this->getRequestFieldName())) {
            try {
                $rule = $this->productRuleRepository->get((int)$entityId);
                $data = $rule->getData();
